	...
	var listVue = new Vue({
		...
		methods : {
			...
			getUserInfoId : function(data){
				listVue.sm_Permission_RangeAuthorization.userInfoId = data.tableId;
			},
			emptyUserInfoId : function(data){
				listVue.sm_Permission_RangeAuthorization.userInfoId = ""
			}
		},
		computed:{
...
//			pageNumber : refresh,
	...
	function getRangeList(rangeAuthType)
	{
		...
		new ServerInterface(baseInfo.rangeListInterface).execute(model, function(jsonObj){
			...
			else
			{
				...
				document.getElementById('sm_Range_AuthorizationForZTEditDiv').scrollIntoView();
			}
		});
	}
	function initZtreeList()
	{
		$(document).ready(function(){
			$.fn.zTree.init($("#treeDemo_RangeAuthForZTEdit"), listVue.setting, listVue.zNodes);
			zTree = $.fn.zTree.getZTreeObj("treeDemo_RangeAuthForZTEdit");
			...
			else
			{
				...
				try
				{
					Vue.nextTick(function () {
						$('#date0101060201').val(jsonObj.sm_Permission_RangeAuthorization.authTimePeriod);
	...
	function saveEdit()
	{
		var model = {
			...
			userInfoId:listVue.sm_Permission_RangeAuthorization.userInfoId,
			rangeAuthType:listVue.sm_Permission_RangeAuthorization.rangeAuthType,
			idArr:listVue.selectItem,
			authTimeStampRange:$('#date0101060201').val(),
		}
		/*console.info(model);
		return;*/
		new ServerInterface(baseInfo.editInterface).execute(model, function(jsonObj){
			...
			else
			{
				...
				generalSuccessModal();
				enterNext2Tab(listVue.tableId, "系统用户范围授权详情", "sm/Sm_Range_AuthorizationForZTDetail.shtml", listVue.tableId+"010106");
			}
		});
	}
	...
	"listDivId":"#sm_Range_AuthorizationForZTEditDiv",
