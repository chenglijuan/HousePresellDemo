...
//			pageNumber : refresh,
	...
	function getRangeList(rangeAuthType)
	{
		...
		new ServerInterface(baseInfo.rangeListInterface).execute(model, function(jsonObj){
			...
			else
			{
				...
				document.getElementById('sm_Range_AuthorizationEditDiv').scrollIntoView();
			}
		});
	}
	function initZtreeList()
	{
		$(document).ready(function(){
			$.fn.zTree.init($("#treeDemo_RangeAuthEdit"), listVue.setting, listVue.zNodes);
			zTree = $.fn.zTree.getZTreeObj("treeDemo_RangeAuthEdit");
			...
			else
			{
				...
				try
				{
					Vue.nextTick(function () {
						$('#date0101050201').val(jsonObj.sm_Permission_RangeAuthorization.authTimePeriod);
	...
	function saveEdit()
	{
		var model = {
			...
			authTimeStampRange:$('#date0101050201').val(),
		}
		/*console.info(model);
		return;*/
		new ServerInterface(baseInfo.editInterface).execute(model, function(jsonObj){
			...
			else
			{
				...
				generalSuccessModal();
				enterNext2Tab(listVue.tableId, "范围授权详情", "sm/Sm_Range_AuthorizationDetail.shtml", listVue.tableId+"010105");
			}
		});
	}
	...
	"listDivId":"#sm_Range_AuthorizationEditDiv",
