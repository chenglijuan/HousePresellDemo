(function (baseInfo) {
    ...
    function getLogDetail(data) {
        ...
        enterNewTab(concatId,"变更日志详情","sm/Sm_BusiState_LogDetail.shtml")
    ...
    function initData() {
        getPoNameFromTab(function (thePoName) {
            listVue.thePoName = thePoName
            console.log('poName is ' + thePoName)
            if(thePoName.indexOf("-")!=-1){
                var poArray=thePoName.split("-")
                listVue.thePoName=poArray[0]
                // listVue.packagePath=poArray[1].replace(/\_/g,".")
                listVue.packagePath=poArray[1]
                listVue.sourceId=poArray[2]
                // console.log('poArray[0] is '+poArray[0])
                // console.log('poArray[1] is '+poArray[1])
                // console.log('poArray[2] is '+poArray[2])
                // console.log('poArray[1] is '+listVue.packagePath)
            }
            refresh()
        })
        // getIdFormTab("", function (thePoName) {
        //     listVue.thePoName = thePoName
        // console.log('poName is '+thePoName)
        // refresh()
        // })
