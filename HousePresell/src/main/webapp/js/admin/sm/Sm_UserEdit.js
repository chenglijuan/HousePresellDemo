	...
	function getDetail()
	{
		new ServerInterface(baseInfo.detailInterface).execute(updateVue.getSearchForm(), function(jsonObj)
		{
			...
			else
			{
				...
				if(updateVue.sm_UserModel.isEncrypt == 0){
					updateVue.sm_UserModel.isEncrypt = "0";
				}else if(updateVue.sm_UserModel.isEncrypt == 1){
					updateVue.sm_UserModel.isEncrypt = "1"
				}
    ...
    function updateSm_User()
    {
    	if(updateVue.sm_Permission_RoleIdArr.length == 0) {
	    	...
	    	new ServerInterfaceJsonBody(baseInfo.updateInterface).execute(updateVue.getUpdateForm(), function(jsonObj){
	    		...
	    		{
	    			generalSuccessModal();
	            	enterNewTabCloseCurrent(updateVue.tableId, '用户详情', 'sm/Sm_UserDetail.shtml')
	    		}
	    	});
    	}
    }
...
//		}
    	...
    	var rv;
		id = TZKeyOcxUserEdit.Tz_SetVenderID(6);
		phKey = TZKeyOcxUserEdit.TZ_Connect(-1,0);
		...
		{
			rv = TZKeyOcxUserEdit.Tz_ReadKeyID(phKey);
