	...
	var listVue = new Vue({
		...
		methods : {
            ...
            isUsingChange:function(data){
            	listVue.sm_Permission_Role.busiType = data.tableId;
            },
            emptyIsUsingChange : function(){
            	listVue.sm_Permission_Role.busiType = null;
            },
		},
		components : {
...
//				console.info(listVue.zNodes);
				...
				$(document).ready(function(){
					$.fn.zTree.init($("#treeDemo_RoleUIEdit"), listVue.setting, listVue.zNodes);
					zTree = $.fn.zTree.getZTreeObj("treeDemo_RoleUIEdit");
					//console.info(zTree.getNodes());
					rMenu = $("#rMenu");
				});
				
				//动态跳转到锚点处，id="top"
				document.getElementById('sm_Permission_RoleUIEditDiv').scrollIntoView();
			}
		});
	}
    ...
    function showDetailMenu()
    {
    	...
    	$('#addOrUpdateMenuDiv_RoleUIEdit').show();
    	$('#noChooseMenuDiv_RoleUIEdit').hide();
    }
    //隐藏菜单项
    function hideDetailMenu()
    {
    	$('#addOrUpdateMenuDiv_RoleUIEdit').hide();
    	$('#noChooseMenuDiv_RoleUIEdit').show();
    }
    ...
    function onClick(event, treeId, treeNode, clickFlag) {
		...
		var zTree = $.fn.zTree.getZTreeObj("treeDemo_RoleUIEdit");
    ...
    function editRoleUI()
    {
    	...
    	new ServerInterface(baseInfo.editInterface).execute(model, function(jsonObj){
			...
			else
			{
				generalSuccessModal();
				listVue.sm_Permission_Role = jsonObj.sm_Permission_Role;
            	enterNewTabCloseCurrent(listVue.sm_Permission_Role.tableId, '角色授权详情', 'sm/Sm_Permission_RoleUIDetail.shtml');
			}
		});
    }
    //------------------------方法定义-结束------------------//
	...
	"listDivId":"#sm_Permission_RoleUIEditDiv",
