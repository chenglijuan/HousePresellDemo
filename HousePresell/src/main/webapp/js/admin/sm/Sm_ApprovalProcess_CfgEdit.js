(function(baseInfo){
	var updateVue = new Vue({
		...
		methods : {
            ...
            isNeedBackupChange : function(data){
            	updateVue.sm_ApprovalProcess_CfgModel.isNeedBackup = data.tableId; 
	      	},
	      	emptyIsNeedBackup : function(){
	      		updateVue.sm_ApprovalProcess_CfgModel.isNeedBackup = null;
	      	}
		},
		computed:{
    ...
    function updateSm_ApprovalProcess_Cfg()
    {
    	new ServerInterface(baseInfo.updateInterface).execute(updateVue.getUpdateForm(), function(jsonObj){
    		...
    		{
                generalSuccessModal();
                enterNewTabCloseCurrent(updateVue.tableId, '审批流程设置详情', 'sm/Sm_ApprovalProcess_CfgDetail.shtml');
    		}
    	});
    }
	//------------------------方法定义-结束------------------//
