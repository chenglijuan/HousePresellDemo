(function (baseInfo) {
    ...
    function selectedItemChanged() {
        listVue.isAllSelected = (listVue.empj_BldLimitAmountList.length > 0)
            && (listVue.selectItem.length == listVue.empj_BldLimitAmountList.length);
    }

    //列表操作--------------“全选”按钮被点击
    function checkAllClicked() {
        if (listVue.selectItem.length == listVue.empj_BldLimitAmountList.length) {
            listVue.selectItem = [];
        } else {
            listVue.selectItem = [];//解决：已经选择一个复选框后，再次点击全选样式问题
            listVue.empj_BldLimitAmountList.forEach(function (item) {
    ...
    function refresh() {
        new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function (jsonObj) {
            if (jsonObj.result != "success") {
                ...
                listVue.empj_BldLimitAmountList = jsonObj.empj_BldLimitAmountList;
                ...
                //动态跳转到锚点处，id="top"
                document.getElementById('Empj_BldLimitAmountListDiv').scrollIntoView();
            }
        });
    }
    ...
    function mainAddHandle() {
        enterNewTab("", "工程进度节点更新增加", "empj/Empj_BldLimitAmountAdd.shtml")
    }
    ...
    function mainEditHandle() {
        ...
        var list = listVue.selectItem
        enterDetail(list, "工程进度节点更新修改", "empj/Empj_BldLimitAmountEdit.shtml")
    }
    ...
    function approvalDetail() {
        ...
        approvalDetailFromHome("工程进度节点更新", listVue.sourceId, listVue.afId, listVue.workflowId, 2);
    }

    function initData() {
    	generalSuccessLimitModal();
        initButtonList();
        getCompanyList();
...
//            }
    ...
    "listDivId": "#Empj_BldLimitAmountListDiv",
    "updateDivId": "#updateModel",
    "addDivId": "#addModal",
    "listInterface": "../Empj_BldLimitAmountList",
    "buildingDetailInterface": "../Empj_BuildingInfoDetail",
    "addInterface": "../Empj_BldLimitAmount_AFAdd",
    "deleteInterface": "../Empj_BldLimitAmountBatchDelete",
