(function (baseInfo) {
    ...
    function selectedItemChanged() {
        listVue.isAllSelected = (listVue.empj_ProjProgForcastList.length > 0)
            && (listVue.selectItem.length == listVue.empj_ProjProgForcastList.length);
    }

    //列表操作--------------“全选”按钮被点击
    function checkAllClicked() {
        if (listVue.selectItem.length == listVue.empj_ProjProgForcastList.length) {
            listVue.selectItem = [];
        } else {
            listVue.selectItem = [];//解决：已经选择一个复选框后，再次点击全选样式问题
            listVue.empj_ProjProgForcastList.forEach(function (item) {
    ...
    function refresh() {
        new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function (jsonObj) {
            if (jsonObj.result != "success") {
                ...
                listVue.empj_ProjProgForcastList = jsonObj.empj_ProjProgForcast_AFList;
                ...
                //动态跳转到锚点处，id="top"
                document.getElementById('Empj_ProjProgForcastListDiv').scrollIntoView();
            }
        });
    }
    ...
    function mainAddHandle() {
        enterNewTab("", "工程进度巡查增加", "empj/Empj_ProjProgForcastAdd.shtml")
    }
    ...
    function mainEditHandle() {
        ...
        var list = listVue.selectItem
        enterDetail(list, "工程进度巡查修改", "empj/Empj_ProjProgForcastEdit.shtml")
    }
    ...
    function approvalDetail() {
        ...
        approvalDetailFromHome("工程进度巡查", listVue.sourceId, listVue.afId, listVue.workflowId, 2);
    }
...
//            }
    ...
    "listDivId": "#Empj_ProjProgForcastListDiv",
    "updateDivId": "#updateModel",
    "addDivId": "#addModal",
    "listInterface": "../Empj_ProjProgForcast_AFList",
    "exportInterface": "../Empj_ProjProgForcast_AFExport",
