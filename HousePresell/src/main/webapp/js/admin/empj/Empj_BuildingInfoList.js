(function(baseInfo){
	var listVue = new Vue({
		...
		data : {
            ...
            escrowStateList : [
            	{tableId:"未托管",theName:"未托管"},
            	{tableId:"已托管",theName:"已托管"},
            	{tableId:"申请托管终止",theName:"申请托管终止"},
            	{tableId:"托管终止",theName:"托管终止"},
            ],
            approvalStateSearchList : [
		...
		methods : {
            ...
            changeEscrowState : function(data){
            ...
            changeLocalProject: function(data) {
            	listVue.localProject = data.tableId;
            },
            changeSaleProject: function(data) {
            	listVue.saleProject = data.tableId;
            }
		},
		computed:{
	...
	{
		generalSelectModal(deleteBuilding, "确认删除");
	}
	...
	{
		enterNewTab("", '新增楼幢', 'empj/Empj_BuildingInfoAdd.shtml')
	}
	...
	{
		enterDetail(listVue.selectItem, "编辑楼幢", "empj/Empj_BuildingInfoEdit.shtml")
	}
	...
	function loadSaleList()
	{
		...
		new ServerInterface(baseInfo.loadLocalInterface).execute(model, function (jsonObj) {
			...
			else
			{
				...
				listVue.projectList.forEach(function(item) {
					item.tableId = item.pROJECTID;
					item.theName = item.pROJECTNAME;
				});
			}
		});
	}
	...
	function getImportList() 
	{
		 ...
		 * 校验是否选择本地项目
		 */
		var localProjectId = listVue.localProject;
		if(undefined == localProjectId || localProjectId == "")
		{
			var jsonObject = {
					info : "请先选择本地项目",
			};
			generalErrorModal(jsonObject);
	...
	function handleSelectionChange1(val){
			...
			for(var i = 0; i < listVue.unImportList.length;i++) {
				if(val[0].eCodeFromConstruction == listVue.unImportList[i].eCodeFromConstruction) {
					listVue.unImportList.splice(i, 1);
				}
			}
