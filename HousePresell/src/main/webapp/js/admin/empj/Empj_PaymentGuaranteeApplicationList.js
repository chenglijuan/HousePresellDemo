(function(baseInfo){
	var listVue = new Vue({
		...
		data : {
			...
			busiState:0,
			empj_PaymentGuaranteeApplicationList:[],
			projectId : "", //项目名称
			companyId : "", //开发企业
	...
	function refresh()
	{
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function(jsonObj){
			...
			{
				listVue.empj_PaymentGuaranteeApplicationList=jsonObj.empj_PaymentGuaranteeList;		
				listVue.empj_PaymentGuaranteeApplicationList.forEach(function(item){
				...
				//动态跳转到锚点处，id="top"
				document.getElementById('Empj_PaymentGuaranteeApplicationListDiv').scrollIntoView();
			}
		});
	}
    ...
    function paymentGuaranteeApplicationAddHandle(){
    	enterNewTab('', '新增支付保证申请', 'empj/Empj_PaymentGuaranteeApplicationAdd.shtml');
/*        $("#tabContainer").data("tabs").addTab({ id: 'Empj_PaymentGuaranteeApplicationAddHandle', text: '新增支付保证申请', closeable: true, url: 'empj/Empj_PaymentGuaranteeApplicationAdd.shtml' });
*/    }
    ...
    function paymentGuaranteeApplicationEditHandle()
    {
        ...
        {
            enterNextTab(listVue.selectItem[0], '修改支付保证申请', 'empj/Empj_PaymentGuaranteeApplicationEdit.shtml',listVue.selectItem[0]+"06120401");
           /* console.log(tableId);
	...
	function paymentGuaranteeApplicationCancelHandle(){
		enterNextTab(listVue.selectItem[0], '撤销支付保证申请', 'empj/Empj_CancelPaymentGuaranteeApplicationEdit.shtml',listVue.selectItem[0]+"06120403");
		/*var tableId = listVue.selectItem[0];
		$("#tabContainer").data("tabs").addTab({ id:"jump_"+tableId, text: '撤销支付保证申请', closeable: true, url: 'empj/Empj_CancelPaymentGuaranteeApplicationAdd.shtml' });*/
	}
	function paymentGuaranteeApplicationDetailHandle(tableId) {
		...
		/*$("#tabContainer").data("tabs").addTab({ id: "jump_"+tableId, text: '支付保证申请详情', closeable: true, url: 'empj/Empj_PaymentGuaranteeApplicationDetail.shtml' });*/
	}
	...
	"listDivId":"#Empj_PaymentGuaranteeApplicationListDiv",
	"companyNameInterface" : "../Emmp_CompanyInfoList", //显示开发企业名称接口
	"changeCompanyInterface" : "../Empj_ProjectInfoList", //改变企业名称接口
	"deleteDivId":"#deletePaymentGuaranteeApplicationList",
	"edModelDivId":"#edModelPaymentGuaranteeApplicationList",
	"sdModelDivId":"#sdModelPaymentGuaranteeApplicationList",
	"listInterface":"../Empj_PaymentGuaranteeApplyList",//列表页面加载接口
	"batchDeleteInterface":"../Empj_PaymentGuaranteeApplyBatchDelete",//批量删除接口
