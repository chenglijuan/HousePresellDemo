(function(baseInfo){
	var listVue = new Vue({
		...
		methods : {
			...
			handlePropelDetail: handlePropelDetail,
			getPropelForm: getPropelForm,
			errClose: errClose,
			succClose: succClose,
			showAccVoucherModal:showAccVoucherModal,
	...
	function getSearchForm()
	{
		return {
			...
			billTimeStamp:document.getElementById("date20040201").value,
		...
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function(jsonObj){
			...
			else
			{
				...
				listVue.tgpf_AccVoucherList.forEach(function(item){
					...
					item.bankTradeAmount = addThousands(item.bankTradeAmount);
				})
				...
				//动态跳转到锚点处，id="top"
				document.getElementById('tgpf_AccVoucherDisbursementListDiv').scrollIntoView();
			}
		});
	}
	...
	function listItemSelectHandle(list) {
        ...
        if(length > 0){
	        for(var i = 0;i < length;i++){
	    		...
	    		listVue.AF_ID = list[i].fundAppropriated_AFId;
	        }
  ...
  //根据按钮权限，动态控制，该页面的所有“可控按钮”的显示与否
	...
	{
		this.keyword = "";
		document.getElementById("date20040201").value = "";
		this.busiState = "";
	}
	//
	function showAccVoucherModal(){
		$("#warningAccVoucherModal").modal('show', {
			backdrop :'static'
		});
	}
	function handlePropel()
	{
		$("#warningAccVoucherModal").modal("hide");
	...
	function handlePropelDetail()
	{
		enterNewTab(listVue.AF_ID, "资金拨付详情", "tgpf/Tgpf_FundAppropriatedDetail.shtml");
	}
	...
	"listDivId":"#tgpf_AccVoucherDisbursementListDiv",
	"errDivId":"#errorF",
	"successDivId":"#successF",
	"listInterface":"../Tgpf_AccVoucherDisbursementList",
	"propelInterface":"../Tgpf_AccVoucherDisbursementPush",
});
