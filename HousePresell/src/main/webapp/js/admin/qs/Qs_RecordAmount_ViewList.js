(function(baseInfo){
	var listVue = new Vue({
		...
		methods : {
			...
			getTheNameofBank: function(data) {
				listVue.theNameofBank = data.tableId;
				listVue.objectname();
			},
			emptyTheNameofBank : function (data){
				listVue.theNameofBank = null;
				listVue.theNameofEscrow = null;
			},
			getTheNameofEscrow: function(data) {
				listVue.theNameofEscrow = data.tableId;
			},
			emptyTheNameofEscrow : function(data){
				listVue.theNameofEscrow =  null;
			}
		},
		computed:{
	...
	{	//list 信息
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function(jsonObj){
			...
			else
			{
				...
				listVue.CheckingAccountList.forEach(function(item){
					item.silverAmount = addThousands(item.silverAmount);
					item.silverNumber = addThousands(item.silverNumber);
					item.escrowAmount = addThousands(item.escrowAmount);
					item.escrowNumber = addThousands(item.escrowNumber);
