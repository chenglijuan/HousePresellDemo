(function(baseInfo){
	var listVue = new Vue({
		...
		methods : {
			...
			getEscrowAcountId: function(data) {
				...
				listVue.escrowAcountName = data.theName;
			},
			emptyEscrowAcountId: function(data) {
				listVue.escrowAcountId = null;
				listVue.escrowAcountName = null;
			},
	...
	/*		getSummaries:function(param) {
		        ...
		        columns.forEach(function(column, index){
		          ...
		          var values = data.map(function(item){Number(item[column.property])});
	...
	function getSearchForm()
	{
		return {
			...
			escrowAcountName : listVue.escrowAcountName,
		}
	}
		...
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function(jsonObj){
			...
			else
			{
				...
				listVue.journalConfirmationReportsList.forEach(function(item){
					...
					item.tradeCount = addThousandsCount(item.tradeCount);
					item.aToatlLoanAmoutCount = addThousandsCount(item.aToatlLoanAmoutCount);
					item.bToatlLoanAmoutCount = addThousandsCount(item.bToatlLoanAmoutCount);
					item.oToatlLoanAmoutCount = addThousandsCount(item.oToatlLoanAmoutCount);
					item.aFristToatlLoanAmoutCount = addThousandsCount(item.aFristToatlLoanAmoutCount);
					item.aToBusinessToatlLoanAmoutCount = addThousandsCount(item.aToBusinessToatlLoanAmoutCount);
				})
