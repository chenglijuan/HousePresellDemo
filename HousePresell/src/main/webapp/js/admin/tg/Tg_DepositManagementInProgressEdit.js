(function(baseInfo){
	...
	function getDetail()
	{
		...
		new ServerInterface(baseInfo.detailInterface).execute(model, function(jsonObj)
		{
			...
			else
			{
				...
				$('#date2101060201').val(editVue.tg_DepositManagementModel.startDateStr);
	...
	function update()
	{
        var model = {
            ...
            startDateStr : $('#date2101060201').val(),
		...
		new ServerInterfaceJsonBody(baseInfo.updateInterface).execute(model, function(jsonObj)
		{
            ...
            {
                generalSuccessModal();
                enterNewTabCloseCurrent(editVue.tableId, '存单正在办理详情', 'tg/Tg_DepositManagementInProgressDetail.shtml');
            }
		});
	}
	...
	"editDivId":"#tg_DepositManagementInProgressEditDiv",
