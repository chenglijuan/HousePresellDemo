(function(baseInfo){
	...
	function getDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(detailVue.getSearchForm(), function(jsonObj)
		{
            ...
            else
            {
				...
				detailVue.emmp_CompanyCooperationModel = jsonObj.emmp_CompanyInfo;
                if(detailVue.emmp_CompanyCooperationModel.busiState == '未备案')
                {
                    detailVue.approvalCode = '020108';
                }
                if(detailVue.emmp_CompanyCooperationModel.approvalState == "审核中")
                {
	...
	//跳转方法
	function companyCooperationEditHandle(tableId) {
        enterNewTabCloseCurrent(tableId, '合作机构修改', 'emmp/Emmp_CompanyCooperationEdit.shtml');
	}
	...
	"detailDivId":"#emmp_CompanyCooperationDetailDiv",
