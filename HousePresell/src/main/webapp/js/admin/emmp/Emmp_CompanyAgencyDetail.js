(function(baseInfo){
	...
	function getDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(detailVue.getSearchForm(), function(jsonObj)
		{
            ...
            {
				detailVue.emmp_CompanyAgencyModel = jsonObj.emmp_CompanyInfo;
                if(detailVue.emmp_CompanyAgencyModel.busiState == '未备案')
                {
                    detailVue.approvalCode = '020103';
                }
                if(detailVue.emmp_CompanyAgencyModel.approvalState == "审核中")
                {
	...
	//跳转方法
	function companyAgencyEditHandle(tableId)
    {
        enterNewTabCloseCurrent(tableId, '代理公司修改', 'emmp/Emmp_CompanyAgencyEdit.shtml');
	}
	...
	"detailDivId":"#emmp_CompanyAgencyDetailDiv",
