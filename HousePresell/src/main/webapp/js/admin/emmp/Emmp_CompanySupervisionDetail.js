(function(baseInfo){
	...
	function getDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(detailVue.getSearchForm(), function(jsonObj)
		{
            ...
            else
            {
				...
				detailVue.emmp_CompanySupervisionModel = jsonObj.emmp_CompanyInfo;
                if(detailVue.emmp_CompanySupervisionModel.busiState == '未备案')
                {
                    detailVue.approvalCode = '020131';
                }
                if(detailVue.emmp_CompanySupervisionModel.approvalState == "审核中")
                {
	...
	//跳转方法
	function CompanySupervisionEditHandle(tableId) {
        enterNewTabCloseCurrent(tableId, '监理机构修改', 'emmp/Emmp_CompanySupervisionEdit.shtml');
	}
	...
	"detailDivId":"#emmp_CompanySupervisionDetailDiv",
