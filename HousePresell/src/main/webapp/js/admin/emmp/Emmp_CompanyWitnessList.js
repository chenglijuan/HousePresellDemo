(function (baseInfo) {
	var listVue = new Vue({
		...
		methods: {
			...
			CompanyWitnessAddHandle: CompanyWitnessAddHandle,
			CompanyWitnessEditHandle: CompanyWitnessEditHandle,
			CompanyWitnessDeleteHandle: CompanyWitnessDeleteHandle,
            CompanyWitnessDel: CompanyWitnessDel,
            CompanyWitnessDelOne: CompanyWitnessDelOne,
			CompanyWitnessDetailHandle: CompanyWitnessDetailHandle,
			CompanyWitnessExportExcelHandle: CompanyWitnessExportExcelHandle,
...
//			countPerPage: refresh,
	...
	function CompanyWitnessEditHandle() {
		if(listVue.selectItem.length == 1) 
		{
            enterDetail(listVue.selectItem, '进度见证服务单位修改', 'emmp/Emmp_CompanyWitnessEdit.shtml');
		} 
		else 
		{
	...
	function CompanyWitnessDeleteHandle() {
		...
		else if (listVue.selectItem.length > 1) 
		{
            ...
            listVue.selectMethod = "CompanyWitnessDel";
		...
		else 
		{
            ...
            listVue.selectMethod = "CompanyWitnessDelOne";
            $(baseInfo.selectM).modal('show', {
                backdrop :'static'
            });
		}
	...
	function CompanyWitnessExportExcelHandle()
	{
	...
	function refresh() {
		...
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function (jsonObj) {
			if (jsonObj.result != "success")
			{
				...
				listVue.emmp_CompanyWitnessList = jsonObj.emmp_CompanyInfoList;
				...
				listVue.selectItem = [];
				//动态跳转到锚点处，id="top"
				document.getElementById('emmp_CompanyWitnessListDiv').scrollIntoView();
			}
		});
	}
...
//     	console.log(column + '-' + column.prop + '-' + column.order)
	...
	//跳转方法
	function CompanyWitnessAddHandle() {
        enterNewTab('', '新增进度见证服务单位', 'emmp/Emmp_CompanyWitnessAdd.shtml');
	}
 
	function CompanyWitnessDetailHandle(tableId) {
        enterNewTab(tableId, '进度见证服务单位详情', 'emmp/Emmp_CompanyWitnessDetail.shtml');
	}
	...
	"listDivId": "#emmp_CompanyWitnessListDiv",
	"listInterface": "../Emmp_CompanyWitnessList",
	"deleteInterface": "../Emmp_CompanyWitnessDelete",
	"batchDeleteInterface": "../Emmp_CompanyWitnessBatchDelete",
	"exportExcelInterface": "../Emmp_CompanyWitnessExportExcel",

	//其他
    "successM": "#companyWitnessList_successM",
    "errorM": "#companyWitnessList_errorM",
    "remindM": "#companyWitnessList_remindM",
    "selectM": "#companyWitnessList_selectM"
});
