(function (baseInfo) {
	var listVue = new Vue({
		...
		methods: {
			...
			CompanyAgencyAddHandle: CompanyAgencyAddHandle,
			CompanyAgencyEditHandle: CompanyAgencyEditHandle,
			CompanyAgencyDeleteHandle: CompanyAgencyDeleteHandle,
            CompanyAgencyDel: CompanyAgencyDel,
            CompanyAgencyDelOne: CompanyAgencyDelOne,
			CompanyAgencyDetailHandle: CompanyAgencyDetailHandle,
			CompanyAgencyExportExcelHandle: CompanyAgencyExportExcelHandle,
...
//			countPerPage: refresh,
	...
	function CompanyAgencyEditHandle() {
		if(listVue.selectItem.length == 1) 
		{
            enterDetail(listVue.selectItem, '代理公司修改', 'emmp/Emmp_CompanyAgencyEdit.shtml');
		} 
		else 
		{
	...
	function CompanyAgencyDeleteHandle() {
		...
		else if (listVue.selectItem.length > 1) 
		{
            ...
            listVue.selectMethod = "CompanyAgencyDel";
		...
		else 
		{
            ...
            listVue.selectMethod = "CompanyAgencyDelOne";
            $(baseInfo.selectM).modal('show', {
                backdrop :'static'
            });
		}
	...
	function CompanyAgencyExportExcelHandle()
	{
	...
	function refresh() {
		...
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function (jsonObj) {
			if (jsonObj.result != "success")
			{
				...
				listVue.emmp_CompanyAgencyList = jsonObj.emmp_CompanyInfoList;
				...
				listVue.selectItem = [];
				//动态跳转到锚点处，id="top"
				document.getElementById('emmp_CompanyAgencyListDiv').scrollIntoView();
			}
		});
	}
...
//     	console.log(column + '-' + column.prop + '-' + column.order)
	...
	//跳转方法
	function CompanyAgencyAddHandle() {
        enterNewTab('', '新增代理公司', 'emmp/Emmp_CompanyAgencyAdd.shtml');
	}
 
	function CompanyAgencyDetailHandle(tableId) {
        enterNewTab(tableId, '代理公司详情', 'emmp/Emmp_CompanyAgencyDetail.shtml');
	}
	...
	"listDivId": "#emmp_CompanyAgencyListDiv",
	"listInterface": "../Emmp_CompanyAgencyList",
	"deleteInterface": "../Emmp_CompanyAgencyDelete",
	"batchDeleteInterface": "../Emmp_CompanyAgencyBatchDelete",
	"exportExcelInterface": "../Emmp_CompanyAgencyExportExcel",

	//其他
    "successM": "#companyAgencyList_successM",
    "errorM": "#companyAgencyList_errorM",
    "remindM": "#companyAgencyList_remindM",
    "selectM": "#companyAgencyList_selectM"
});
