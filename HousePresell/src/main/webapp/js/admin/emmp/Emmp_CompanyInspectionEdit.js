(function(baseInfo){
	...
	function getCompanyInspectionDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(editVue.getSearchForm(), function(jsonObj)
		{
            ...
            else
            {
				...
				editVue.emmp_CompanyInspectionModel = jsonObj.emmp_CompanyInfo;
                editVue.oldObj = jsonObj.emmp_CompanyInfo.oldObj;
                $('#date0201410201').val(editVue.emmp_CompanyInspectionModel.registeredDateStr);
                editVue.orgMemberList = jsonObj.emmp_OrgMemberList;
                if(editVue.emmp_CompanyInspectionModel.busiState == '未备案')
                {
                    editVue.approvalCode = '020141';
                }
			}
		});
	}
	...
	function update(buttonType)
	{
		...
		var model = {
			...
			theType:this.emmp_CompanyInspectionModel.theType,
			companyGroup:this.emmp_CompanyInspectionModel.companyGroup,
			theName:this.emmp_CompanyInspectionModel.theName,
			registeredDateStr:$('#date0201410201').val(),
			qualificationGrade:this.emmp_CompanyInspectionModel.qualificationGrade,
			legalPerson:this.emmp_CompanyInspectionModel.legalPerson,
			unifiedSocialCreditCode:this.emmp_CompanyInspectionModel.unifiedSocialCreditCode,

            contactPerson : this.emmp_CompanyInspectionModel.contactPerson,
            contactPhone : this.emmp_CompanyInspectionModel.contactPhone,

			address:this.emmp_CompanyInspectionModel.address,
	    	busiCode:this.busiCode,

            busiState:this.emmp_CompanyInspectionModel.busiState,

            isUsedState:this.emmp_CompanyInspectionModel.isUsedState,
            ...
            approvalState : (buttonType == '2') ? '审核中' : this.emmp_CompanyInspectionModel.approvalState
		};
		...
		new ServerInterfaceJsonBody(baseInfo.updateInterface).execute(model, function(jsonObj)
		{
            ...
            {
                generalSuccessModal();
                enterNewTabCloseCurrent(editVue.tableId, '无人机巡查机构详情', 'emmp/Emmp_CompanyInspectionDetail.shtml');
			}
		});
	}
	...
	function refresh()
	{
        ...
        getCompanyInspectionDetail();		//获取详情
	...
	"editDivId":"#emmp_CompanyInspectionEditDiv",
	"detailInterface":"../Emmp_CompanyInspectionDetail",
	"updateInterface":"../Emmp_CompanyInspectionUpdate",
