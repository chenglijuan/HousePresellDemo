(function (baseInfo) {
	var listVue = new Vue({
		...
		methods: {
			...
			CompanySupervisionAddHandle: CompanySupervisionAddHandle,
			CompanySupervisionEditHandle: CompanySupervisionEditHandle,
			CompanySupervisionDeleteHandle: CompanySupervisionDeleteHandle,
            CompanySupervisionDel: CompanySupervisionDel,
            CompanySupervisionDelOne: CompanySupervisionDelOne,
			CompanySupervisionDetailHandle: CompanySupervisionDetailHandle,
			CompanySupervisionExportExcelHandle: CompanySupervisionExportExcelHandle,
...
//			countPerPage: refresh,
	...
	function CompanySupervisionEditHandle() {
		if(listVue.selectItem.length == 1) 
		{
            enterDetail(listVue.selectItem, '监理机构修改', 'emmp/Emmp_CompanySupervisionEdit.shtml');
		} 
		else 
		{
	...
	function CompanySupervisionDeleteHandle() {
		...
		else if (listVue.selectItem.length > 1) 
		{
            ...
            listVue.selectMethod = "CompanySupervisionDel";
		...
		else 
		{
            ...
            listVue.selectMethod = "CompanySupervisionDelOne";
            $(baseInfo.selectM).modal('show', {
                backdrop :'static'
            });
		}
	...
	function CompanySupervisionDel() {
	...
	function CompanySupervisionDelOne() {
		var model = {
			interfaceVersion: 19000101,
	...
	function CompanySupervisionExportExcelHandle()
	{
	...
	function refresh() {
		...
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function (jsonObj) {
			if (jsonObj.result != "success")
			{
				...
				listVue.emmp_CompanySupervisionList = jsonObj.emmp_CompanyInfoList;
				...
				listVue.selectItem = [];
				//动态跳转到锚点处，id="top"
				document.getElementById('emmp_CompanySupervisionListDiv').scrollIntoView();
			}
		});
	}
...
//     	console.log(column + '-' + column.prop + '-' + column.order)
	...
	//跳转方法
	function CompanySupervisionAddHandle() {
        enterNewTab('', '新增监理机构', 'emmp/Emmp_CompanySupervisionAdd.shtml');
	}
 
	function CompanySupervisionDetailHandle(tableId) {
        enterNewTab(tableId, '监理机构详情', 'emmp/Emmp_CompanySupervisionDetail.shtml');
	}
	...
	"listDivId": "#emmp_CompanySupervisionListDiv",
	"listInterface": "../Emmp_CompanySupervisionList",
	"deleteInterface": "../Emmp_CompanySupervisionDelete",
	"batchDeleteInterface": "../Emmp_CompanySupervisionBatchDelete",
	"exportExcelInterface": "../Emmp_CompanySupervisionExportExcel",

	//其他
    "successM": "#CompanySupervisionList_successM",
    "errorM": "#CompanySupervisionList_errorM",
    "remindM": "#CompanySupervisionList_remindM",
    "selectM": "#CompanySupervisionList_selectM"
});
