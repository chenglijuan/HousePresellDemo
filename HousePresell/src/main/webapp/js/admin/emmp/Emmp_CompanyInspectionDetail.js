(function(baseInfo){
	...
	function getDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(detailVue.getSearchForm(), function(jsonObj)
		{
            ...
            else
            {
				...
				detailVue.emmp_CompanyInspectionModel = jsonObj.emmp_CompanyInfo;
                if(detailVue.emmp_CompanyInspectionModel.busiState == '未备案')
                {
                    detailVue.approvalCode = '020141';
                }
                if(detailVue.emmp_CompanyInspectionModel.approvalState == "审核中")
                {
	...
	//跳转方法
	function CompanyInspectionEditHandle(tableId) {
        enterNewTabCloseCurrent(tableId, '无人机巡查机构修改', 'emmp/Emmp_CompanyInspectionEdit.shtml');
	}
	...
	"detailDivId":"#emmp_CompanyInspectionDetailDiv",
