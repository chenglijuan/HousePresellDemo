(function(baseInfo){
	var editVue = new Vue({
		...
		methods : {
			...
			getIsUsedState : function(data){
				editVue.emmp_CompanyAgencyModel.isUsedState = data.tableId;
			},
			emptyIsUsedState : function(){
				editVue.emmp_CompanyAgencyModel.isUsedState = null;
			},
	...
	function getCompanyAgencyDetail()
	{
		new ServerInterfaceSync(baseInfo.detailInterface).execute(editVue.getSearchForm(), function(jsonObj)
		{
            ...
            else
            {
				...
				editVue.emmp_CompanyAgencyModel = jsonObj.emmp_CompanyInfo;
                editVue.oldObj = jsonObj.emmp_CompanyInfo.oldObj;
                $('#date0201030201').val(editVue.emmp_CompanyAgencyModel.registeredDateStr);
                editVue.orgMemberList = jsonObj.emmp_OrgMemberList;
                if(editVue.emmp_CompanyAgencyModel.busiState == '未备案')
                {
                    editVue.approvalCode = '020103';
                }
			}
		});
	}
	...
	function update(buttonType)
	{
		...
		var model = {
            ...
            theType:this.emmp_CompanyAgencyModel.theType,
            companyGroup:this.emmp_CompanyAgencyModel.companyGroup,
            theName:this.emmp_CompanyAgencyModel.theName,
            registeredDateStr:$('#date0201030201').val(),
            qualificationGrade:this.emmp_CompanyAgencyModel.qualificationGrade,
            legalPerson:this.emmp_CompanyAgencyModel.legalPerson,
            unifiedSocialCreditCode:this.emmp_CompanyAgencyModel.unifiedSocialCreditCode,

            contactPerson : this.emmp_CompanyAgencyModel.contactPerson,
            contactPhone : this.emmp_CompanyAgencyModel.contactPhone,

            address:this.emmp_CompanyAgencyModel.address,
            busiCode:this.busiCode,

            busiState:this.emmp_CompanyAgencyModel.busiState,

            isUsedState:this.emmp_CompanyAgencyModel.isUsedState,
            ...
            approvalState : (buttonType == '2') ? '审核中' : this.emmp_CompanyAgencyModel.approvalState
		};
		...
		new ServerInterfaceJsonBody(baseInfo.updateInterface).execute(model, function(jsonObj)
		{
            ...
            {
                generalSuccessModal();
                enterNewTabCloseCurrent(editVue.tableId, '代理公司详情', 'emmp/Emmp_CompanyAgencyDetail.shtml');
			}
		});
	}
	...
	"editDivId":"#emmp_CompanyAgencyEditDiv",
	"detailInterface":"../Emmp_CompanyAgencyDetail",
	"updateInterface":"../Emmp_CompanyAgencyUpdate",
