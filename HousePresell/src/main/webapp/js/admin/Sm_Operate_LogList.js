...
//			pageNumber : refresh,
	...
	{	
		listVue.isAllSelected = (listVue.sm_Operate_LogList.length > 0)
		&&	(listVue.selectItem.length == listVue.sm_Operate_LogList.length);
	}
	//列表操作--------------“全选”按钮被点击
	function checkAllClicked()
	{
	    if(listVue.selectItem.length == listVue.sm_Operate_LogList.length)
	    {
	    	listVue.selectItem = [];
	    }
	    else
	    {
	    	...
	    	listVue.sm_Operate_LogList.forEach(function(item) {
	...
	function refresh()
	{
		new ServerInterface(baseInfo.listInterface).execute(listVue.getSearchForm(), function(jsonObj){
			...
			{
				listVue.sm_Operate_LogList=jsonObj.sm_Operate_LogList;
				...
				//动态跳转到锚点处，id="top"
				document.getElementById('sm_Operate_LogListDiv').scrollIntoView();
			}
		});
	}
	...
	"listDivId":"#sm_Operate_LogListDiv",
	"updateDivId":"#updateModel",
	"addDivId":"#addModal",
	"listInterface":"../Sm_Operate_LogList",
