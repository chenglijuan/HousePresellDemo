 ...
 * 弹出成功模态框
 */
function generalSuccessLimitModal() {
    $("#sm_dialog").modal('show',{
        backdrop :'static'
    });
}
 ...
 */
function generalLoadFile2(vue, busiType) {
    var form = {
        pageNumber:"0",
        busiType : busiType,
        interfaceVersion:vue.interfaceVersion,
    ...
    serverRequest("../Sm_AttachmentCfgList",form,function (jsonObj) {
        vue.loadUploadList= jsonObj.sm_AttachmentCfgList;
        //console.log(vue.loadUploadList);
    },function (jsonObj) {
        generalErrorModal(jsonObj)
    })
}
...
function checkBtnNeedShow(jsonObj)
{
    ...
    $('[data-editNum]').each(function(){
    	//console.info($(this).attr("data-editNum"));
        ...
        {
            if(needShowBtnList.indexOf($(this).attr("data-editNum")) > -1)
            {
                //显示
                $(this).show();
            }
    ...
    $('[data-showNum]').each(function(){
        ...
        {
            if(needShowBtnList.indexOf($(this).attr("data-showNum")) > -1)
            {
            	 //显示
                $(this).show();
            }
    ...
    $('[data-editTime]').each(function(){
        ...
        {
            if(needShowBtnList.indexOf($(this).attr("data-editTime")) > -1)
            {
            	 //显示
                $(this).show();
            }else{
...
function enterLogTab(poName,packagePath,listVue) {
    ...
    enterNewTab(poName,"变更日志","sm/Sm_BusiState_LogList.shtml")
    // enterNewTab(poName,"变更日志","sm/Sm_BusiState_LogDetail.shtml")
}
...
// html命名只能包含两个下划线
function approvalDetailFromHome(busiType,sourceId,afId,workflowId,sourcePage) {
    var theId = sourceId + "_" + afId + "_" + workflowId+"_"+sourcePage;
    console.log(theId);
    console.log(busiType);
    switch (busiType) {
        case "开发企业注册":
            enterNewTab(theId, "开发企业注册审批", "sm/Sm_ApprovalProcess_CompanyInfoDetail.shtml");
            break;
        case "开发企业变更":
            enterNewTab(theId, "开发企业变更审批", "sm/Sm_ApprovalProcess_CompanyInfoDetail.shtml");
            break;
        case "代理公司注册":
            enterNewTab(theId, "代理公司注册审批", "sm/Sm_ApprovalProcess_CompanyAgencyDetail.shtml");
            break;
        case "代理公司变更":
            enterNewTab(theId, "代理公司变更审批", "sm/Sm_ApprovalProcess_CompanyAgencyDetail.shtml");
            break;
        case "进度见证服务单位注册":
            enterNewTab(theId, "进度见证服务单位注册审批", "sm/Sm_ApprovalProcess_CompanyWitnessDetail.shtml");
            break;
        case "进度见证服务单位变更":
            enterNewTab(theId, "进度见证服务单位变更审批", "sm/Sm_ApprovalProcess_CompanyWitnessDetail.shtml");
            break;
        case "合作机构注册":
            enterNewTab(theId, "合作机构注册审批", "sm/Sm_ApprovalProcess_CompanyCooperationDetail.shtml");
            break;
        case "合作机构变更":
            enterNewTab(theId, "合作机构变更审批", "sm/Sm_ApprovalProcess_CompanyCooperationDetail.shtml");
            break;
        case "项目信息注册与备案":
            enterNewTab(theId, "项目信息注册与备案", "sm/Sm_ApprovalProcess_ProjectInfoDetail.shtml");
            break;
        case "项目信息变更与备案":
            enterNewTab(theId, "项目信息变更与备案", "sm/Sm_ApprovalProcess_ProjectInfoDetail.shtml");
            break;
        case "楼幢信息变更维护":
            enterNewTab(theId, "楼幢信息变更维护审批", "sm/Sm_ApprovalProcess_BuildingDetail.shtml");
            break;
        case "楼幢信息初始维护（含楼幢、单元、户室信息维护）":
            enterNewTab(theId, "楼幢信息初始维护审批", "sm/Sm_ApprovalProcess_BuildingDetail.shtml");
            break;
        case "托管终止":
            enterNewTab(theId, "托管终止", "sm/Sm_ApprovalProcess_BldEscrowCompletedDetail.shtml");
            break;
        case "托管标准版本管理":
            enterNewTab(theId, "托管标准版本管理", "sm/Sm_ApprovalProcess_EscrowStandardVerMngDetail.shtml");
            break;
        case "退房退款申请-贷款已结清":
            enterNewTab(theId, "退房退款申请审批", "sm/Sm_RefundInfoDetail.shtml");
            break;
        case "贷款托管合作协议签署":
            enterNewTab(theId, "贷款托管合作协议签署审批", "sm/Sm_EscrowAgreementDetail.shtml");
            break;
        case "贷款三方托管协议签署":
            enterNewTab(theId, "贷款三方托管协议签署审批", "sm/Sm_TripleAgreementDetail.shtml");
            break;
        case "退房退款申请-贷款未结清":
            enterNewTab(theId, "退房退款申请-贷款未结清审批", "sm/Sm_RefundInfoUnclearedDetail.shtml");
            break;
        case "支付保证申请与复核":
            enterNewTab(theId, "支付保证申请与复核审批", "sm/Sm_PaymentGuaranteeApplicationDetail.shtml");
            break;
        case "支付保函申请与复核":
            enterNewTab(theId, "支付保函申请与复核", "sm/Sm_ApprovalProcess_PaymentBondDetail.shtml");
            break;
        case "支付保证撤销":
            enterNewTab(theId, "支付保证撤销审批", "sm/Sm_CancelPaymentGuaranteeApplicationDetail.shtml");
            break;

        case "三方协议版本管理":
            enterNewTab(theId, "三方协议版本管理审批", "sm/Sm_TripleAgreementVerMngDetail.shtml");
            break;

        case "三方协议计量结算":
            enterNewTab(theId, "三方协议计量结算审批", "sm/Sm_CoopAgreementSettleDtlDetail.shtml");
            break;

        case "合作协议版本管理":
            enterNewTab(theId, "合作协议版本管理审批", "sm/Sm_CoopAgreementVerMngDetail.shtml");
            break;

        case "用款申请与复核":
            enterNewTab(theId, "用款申请与复核审批", "sm/Sm_ApprovalProcess_FundAppropriatedAFDetail.shtml");
            break;

        case "统筹与复核":
            enterNewTab(theId, "统筹与复核审批", "sm/Sm_ApprovalProcess_FundOverallPlanDetail.shtml");
            break;

        case "资金拨付":
            enterNewTab(theId, "资金拨付审批", "sm/Sm_ApprovalProcess_FundAppropriatedDetail.shtml");
            break;

        case "备案价格初始维护":
            enterNewTab(theId, "备案价格初始维护审批", "sm/Sm_ApprovalProcess_AvgPriceDetail.shtml");
            break;

        case "备案价格变更维护":
            enterNewTab(theId, "备案价格变更维护审批", "sm/Sm_ApprovalProcess_AvgPriceDetail.shtml");
            break;

        case "受限额度变更":
            enterNewTab(theId, "受限额度变更", "sm/Sm_ApprovalProcess_BldLimitAmountDetail.shtml");
            break;
        case "受限额度节点版本管理":
            enterNewTab(theId, "受限额度节点版本管理", "sm/Sm_ApprovalProcess_BldLimitAmountVerDetail.shtml");
            break;

        case "存单存入管理":
            enterNewTab(theId, "存单存入审批", "sm/Sm_ApprovalProcess_DepositManagementDetail.shtml");
            break;
        case "存单提取管理":
            enterNewTab(theId, "存单提取审批", "sm/Sm_ApprovalProcess_DepositManagementTakeOutDetail.shtml");
            break;
        case "托管特殊拨付管理":
            enterNewTab(theId, "托管特殊拨付管理审批", "sm/Sm_SpecialFundAppropriatedDetail.shtml");
            break;
        case "工程进度节点更新":
            enterNewTab(theId, "工程进度节点更新审批", "sm/Sm_BldLimitAmountDetail.shtml");
            break;
        case "工程进度巡查":
            enterNewTab(theId, "工程进度巡查审批", "sm/Sm_ProjProgForcastDetail.shtml");
            break;
        case "工程进度巡查推送管理":
            enterNewTab(theId, "工程进度巡查推送审批", "sm/Sm_ProjProgForcastManageDetail.shtml");
            break;
        default:
    }
}
