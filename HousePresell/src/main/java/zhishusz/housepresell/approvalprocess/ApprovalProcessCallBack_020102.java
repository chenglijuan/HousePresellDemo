 ...
 */
@Transactional
public class ApprovalProcessCallBack_020102 implements IApprovalProcessCallback
{
	@Autowired
	...
	public Properties execute(Sm_ApprovalProcess_Workflow approvalProcessWorkflow, BaseForm model)
	{
		...
		try
		{
			...
			Emmp_CompanyInfoTemplate companyOldTemplate = companyLogUitl.getCopyTemplate(emmp_companyInfo, emmp_OrgMemberList);
				...
				case "020102001_ZS":
					if(S_ApprovalState.Completed.equals(sm_ApprovalProcess_AF.getBusiState()) && S_WorkflowBusiState.Completed.equals(approvalProcessWorkflow.getBusiState()))
					{
						...
						if(jsonStr != null && jsonStr.length() > 0)
						{
							...
							emmp_companyInfo.setRegisteredDate(emmp_companyInfoForm.getRegisteredDate());
							...
							Long cityRegionId = emmp_companyInfoForm.getCityRegionId();
							if (cityRegionId != null && cityRegionId > 0)
							{
								emmp_companyInfo.setCityRegion(sm_CityRegionInfoDao.findById(cityRegionId));
							}
							Long streetId = emmp_companyInfoForm.getStreetId();
							if (streetId != null && streetId > 0)
							{
								emmp_companyInfo.setStreet(sm_StreetInfoDao.findById(streetId));
							}
							...
							emmp_companyInfo.setShortName(emmp_companyInfoForm.getShortName());
							...
							Emmp_CompanyInfoTemplate companyNewTemplate = companyLogUitl.getCopyTemplate(emmp_companyInfo, emmp_orgMemberListNew);
							...
							//更新附件信息
							emmp_companyInfoForm.setBusiType("020101");
