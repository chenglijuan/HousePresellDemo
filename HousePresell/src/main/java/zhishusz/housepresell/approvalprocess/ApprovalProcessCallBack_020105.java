 ...
 */
@Transactional
public class ApprovalProcessCallBack_020105 implements IApprovalProcessCallback
{
	@Autowired
	...
	public Properties execute(Sm_ApprovalProcess_Workflow approvalProcessWorkflow, BaseForm model)
	{
		...
		try
		{
			...
			{
				return MyBackInfo.fail(properties, "审批的代理公司不存在");
			}
			...
			Emmp_CompanyInfoTemplate companyOldTemplate = companyLogUitl.getCopyTemplate(emmp_companyInfo, emmp_OrgMemberList, S_CompanyType.Agency);
				...
				case "020105001_ZS":
					...
					if(S_ApprovalState.Completed.equals(sm_ApprovalProcess_AF.getBusiState()) && S_WorkflowBusiState.Completed.equals(approvalProcessWorkflow.getBusiState()))
					{
						...
						if (jsonStr != null && jsonStr.length() > 0)
						{
							...
							Emmp_CompanyInfoTemplate companyNewTemplate = companyLogUitl.getCopyTemplate(emmp_companyInfo, emmp_orgMemberListNew, S_CompanyType.Agency);
							...
							//更新附件信息
							emmp_companyInfoForm.setBusiType("020103");
