...
import zhishusz.housepresell.approvalprocess.IApprovalProcessCallback;
...
@Service
@Transactional
public class ApprovalProcessCallBack_06120202 implements IApprovalProcessCallback
{
	...
	public Properties execute(Sm_ApprovalProcess_Workflow approvalProcessWorkflow, BaseForm baseForm)
	{
		...
		if (S_ApprovalState.NoPass.equals(sm_ApprovalProcess_AF.getBusiState()))
		{
			...
			Tgxy_TripleAgreement tripleAgreement = tgxy_TripleAgreementDao.findOneByQuery_T(
					tgxy_TripleAgreementDao.getQuery(tgxy_TripleAgreementDao.getBasicHQL(), tripleAgreementForm));
		...
		if (null != action && 0 == action && approvalProcessWork.equals("06120202001_TC"))
		{
		...
		if (null != action && 0 == action && approvalProcessWork.equals("06120202001_ZS"))
		{
		...
		if (approvalProcessWork.equals("06120202001_ZS"))
		{
					...
					&& S_WorkflowBusiState.Completed.equals(approvalProcessWorkflow.getBusiState()))
			{
				...
				/*
				 * 更新楼幢账户表
				 * 计算：增加“已退款金额（元）”、减少“已申请退款未拨付金额（元）”、减少“当前托管余额（元）”、减少“溢出金额（元）
				 * 减少冻结金额
				 */
				// 已退款金额
				Double refundAmount = tgpj_BuildingAccount.getRefundAmount();
...
//				}
		...
		else if(approvalProcessWork.equals("06120202001_4"))
        {
            properties.put(S_NormalFlag.result, S_NormalFlag.success);
            properties.put(S_NormalFlag.info, S_NormalFlag.info_Success);
