...
import zhishusz.housepresell.service.Emmp_CompanyAgencyDetailService;
...
@Controller
@EnableCaching
public class Emmp_CompanyAgencyDetailController extends BaseController
{
	@Autowired
	private Emmp_CompanyAgencyDetailService service;
	...
	@RequestMapping(value="/Emmp_CompanyAgencyDetail",produces="text/html;charset=UTF-8",method={RequestMethod.GET,RequestMethod.POST})
	@ResponseBody
//	@Cacheable(value="Emmp_CompanyAgency", key="'Emmp_CompanyInfoId'+#model.getTableId()")
		...
		if(MyBackInfo.isSuccess(properties))
		{
			...
			else
			{
				...
				properties.put("emmp_OrgMemberList", rebuild2.getListForApproval((List)(properties.get(
						"emmp_OrgMemberList")), model.getTableId(), S_BusiState.NoRecord.equals(model.getBusiState()) ? "020103" : "020105", model.getBusiState()));
			}
		}
		
		String jsonStr = new JsonUtil().propertiesToJson(properties);
		
		super.writeOperateHistory("Emmp_CompanyAgencyDetail", model, properties, jsonStr);
