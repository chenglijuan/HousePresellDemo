...
/*
 * Service更新操作：推送给财务系统-设置
 * Company：ZhiShuSZ
 * */
@Service
@Transactional
public class Tgpf_FundAccountInfoUpdateService
{
	@Autowired
...
//		Integer operateType = model.getOperateType();
		String depositRemark = model.getDepositRemark();
		String financeRemark = model.getFinanceRemark();
...
//		}
		...
		{
			return MyBackInfo.fail(properties, "财务系统企业名称不能为空");
		}
...
//		}
		...
		{
			return MyBackInfo.fail(properties, "财务系统项目名称不能为空");
		}
...
//		}
		...
		{
			return MyBackInfo.fail(properties, "财务系统楼幢全称不能为空");
		}
		if(shortNameOfBuildingFromFinanceSystem == null || shortNameOfBuildingFromFinanceSystem.length() == 0)
		{
			return MyBackInfo.fail(properties, "财务系统楼幢简称不能为空");
		}
...
//	
		...
		{
			return MyBackInfo.fail(properties, "该对照记录不存在！");
		}
...
//		tgpf_FundAccountInfo.setCreateTimeStamp(createTimeStamp);
		tgpf_FundAccountInfo.setUserUpdate(user);
...
//		tgpf_FundAccountInfo.setOperateType(operateType);
		tgpf_FundAccountInfo.setDepositRemark(depositRemark);
		tgpf_FundAccountInfo.setFinanceRemark(financeRemark);
//		tgpf_FundAccountInfo.setConfigureUser();
