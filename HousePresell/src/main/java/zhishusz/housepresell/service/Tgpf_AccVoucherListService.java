...
public class Tgpf_AccVoucherListService
{
	@Autowired
	...
	public Properties execute(Tgpf_AccVoucherForm model)
	{
		...
		model.setTradeCount(0);
		
		Integer pageNumber = Checker.getInstance().checkPageNumber(model.getPageNumber());
		Integer countPerPage = Checker.getInstance().checkCountPerPage(model.getCountPerPage());
		
		Integer totalCount = tgpf_AccVoucherDao.findByPage_Size(tgpf_AccVoucherDao.getQuery_Size(tgpf_AccVoucherDao.getSpeicalHQL(), model));
		...
		if(totalCount > 0)
		{
			tgpf_AccVoucherList = tgpf_AccVoucherDao.findByPage(tgpf_AccVoucherDao.getQuery(tgpf_AccVoucherDao.getSpeicalHQL(), model), pageNumber, countPerPage);
		}
		else
		{
