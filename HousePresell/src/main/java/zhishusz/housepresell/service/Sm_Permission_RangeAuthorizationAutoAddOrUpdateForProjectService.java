...
@Service
@Transactional
public class Sm_Permission_RangeAuthorizationAutoAddOrUpdateForProjectService
{
private MyDatetime myDatetime = MyDatetime.getInstance();
	...
	public Properties execute(Sm_Permission_RangeAuthorizationForm model)
	{
		...
		Integer rangeAuthType = S_RangeAuthType.Project;
		...
		Long projectInfoId = model.getProjectInfoId();
		Empj_ProjectInfo empj_ProjectInfo = empj_ProjectInfoDao.findById(projectInfoId);
		if(empj_ProjectInfo == null)
		{
			return MyBackInfo.fail(properties, "该项目不存在");
		}
		
		Emmp_CompanyInfo emmp_CompanyInfo = empj_ProjectInfo.getDevelopCompany();
