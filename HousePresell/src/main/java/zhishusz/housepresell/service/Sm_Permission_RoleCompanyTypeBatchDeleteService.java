...
/*
 * Service批量删除：角色与机构类型对应关系
 * Company：ZhiShuSZ
 * */
@Service
@Transactional
public class Sm_Permission_RoleCompanyTypeBatchDeleteService
{
	@Autowired
	...
	public Properties execute(Sm_Permission_RoleCompanyTypeForm model)
	{
		...
		{
			Sm_Permission_RoleCompanyType sm_Permission_RoleCompanyType = (Sm_Permission_RoleCompanyType)sm_Permission_RoleCompanyTypeDao.findById(tableId);
			if(sm_Permission_RoleCompanyType == null)
			{
				return MyBackInfo.fail(properties, "'Sm_Permission_RoleCompanyType(Id:" + tableId + ")'不存在");
			}
