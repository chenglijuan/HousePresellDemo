...
/*
 * Service列表查询：正泰机构范围授权删除
 * Company：ZhiShuSZ
 * */
@Service
@Transactional
public class Sm_Permission_RangeAuthorizationForZTDeleteService
{
	@Autowired
	...
	public Properties execute(Sm_Permission_RangeAuthorizationForm model)
	{
		...
		for(Long id : idArr)
		{
			...
			//删除该正泰用户原先所有的已经分配的数据
			sm_Permission_RangeDao.deleteBatch(sm_Permission_RangeAuthorization.getUserInfo(), null);
