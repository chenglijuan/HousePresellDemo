...
@Service
@Transactional
public class Emmp_CompanyCooperationUpdateService
{
	private static final String ADD_BUSI_CODE 	= "020108"; //代理公司新增
	private static final String UPDATE_BUSI_CODE= "020109"; //代理公司变更
	...
	public Properties execute(Emmp_CompanyInfoForm model)
	{
		...
		{
			return MyBackInfo.fail(properties, "合作机构不存在");
		}
		...
		{
			return MyBackInfo.fail(properties, "请输入合作机构名称");
		}
		...
		if (checkNameCount > 0)
		{
			return MyBackInfo.fail(properties, "合作机构名称已被占用");
		}

//		if (address == null || address.length() == 0)
//		{
//			return MyBackInfo.fail(properties, "请输入合作机构地址");
//		}
...
//			return MyBackInfo.fail(properties, "请输入统一社会信用代码");
		...
		Emmp_CompanyInfoTemplate companyOldTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListOrg, S_CompanyType.Cooperation);
...
//		model.setOrgMemberList(orgMemberList);
		...
		else if (!flag)
		{
			...
			if ("noApproval".equals(properties.getProperty(S_NormalFlag.info)))
			{
				...
				Emmp_CompanyInfoTemplate companyNewTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListNew, S_CompanyType.Cooperation);
