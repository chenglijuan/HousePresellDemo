...
@Service
@Transactional
public class Tgpj_BuildingAccountLimitedUpdateService
{
...
//					if("物价备案均价30%".equals(escrowStandardVerMng.getTheName()))
		...
		// 从log表中 获取tableId
		Tgpj_BuildingAccount tgpj_BuildingAccount = tgpj_BuildingAccountLog.getTgpj_BuildingAccount();
		if(tgpj_BuildingAccount == null)
		{
			return MyBackInfo.fail(properties, "楼幢账户信息不存在");
		}
		
		//更新楼幢账户表中的数据(直接覆盖的)
		tgpj_BuildingAccount.setEscrowStandard(tgpj_BuildingAccountLog.getEscrowStandard());
		tgpj_BuildingAccount.setEscrowArea(tgpj_BuildingAccountLog.getEscrowArea());
		tgpj_BuildingAccount.setBuildingArea(tgpj_BuildingAccountLog.getBuildingArea());
		...
		tgpj_BuildingAccount.setTotalGuaranteeAmount(tgpj_BuildingAccountLog.getTotalGuaranteeAmount());
...
//		tgpj_BuildingAccount.setRecordAvgPriceOfBuildingFromPresellSystem(tgpj_BuildingAccountLog.getRecordAvgPriceOfBuildingFromPresellSystem());
		tgpj_BuildingAccount.setRecordAvgPriceOfBuilding(recordAvgPriceOfBuilding);
		tgpj_BuildingAccount.setActualAmount(tgpj_BuildingAccountLog.getActualAmount());
		tgpj_BuildingAccount.setPaymentLines(paymentLines);
		tgpj_BuildingAccount.setPaymentProportion(paymentProportion);
		tgpj_BuildingAccount.setBuildAmountPaid(buildAmountPaid);	
		tgpj_BuildingAccount.setBuildAmountPay(buildAmountPay);
		tgpj_BuildingAccount.setTotalAmountGuaranteed(totalAmountGuaranteed);
		tgpj_BuildingAccount.setBldLimitAmountVerDtl(tgpj_BuildingAccountLog.getBldLimitAmountVerDtl());
			
		// 更新楼幢账户表中的数据(需要重新计算的)
		tgpj_BuildingAccount.setCashLimitedAmount(cashLimitedAmount);//现金受限额度
		tgpj_BuildingAccount.setEffectiveLimitedAmount(effectiveLimitedAmount); // 有效受限额度
		tgpj_BuildingAccount.setSpilloverAmount(spilloverAmount);// 溢出金额
		tgpj_BuildingAccount.setCurrentEscrowFund(currentEscrowFund);// 托管余额
		tgpj_BuildingAccount.setAllocableAmount(releaseTheAmount);// 可拨付金额
	  
		// 更新版本号
		Long versionNo ;
		if( null == tgpj_BuildingAccountLog.getVersionNo() || tgpj_BuildingAccountLog.getVersionNo() < 0)
		{
			versionNo = 1l;
		}
		else
		{
			versionNo = tgpj_BuildingAccountLog.getVersionNo() + 1;
		}
		tgpj_BuildingAccount.setVersionNo(versionNo);
		
		tgpj_BuildingAccountDao.save(tgpj_BuildingAccount);
		
		tgpj_BuildingAccountLog.setTgpj_BuildingAccount(tgpj_BuildingAccount);
		tgpj_BuildingAccountLog.setBusiState("1"); // 启用状态
	...
	/**
	 * 用于楼幢价格初始维护后的计算调用
	 * @param tgpj_BuildingAccountLog
	 * @return
	 */
	public Properties execute1(Tgpj_BuildingAccountLog tgpj_BuildingAccountLog)
	{
		Properties properties = new MyProperties();
