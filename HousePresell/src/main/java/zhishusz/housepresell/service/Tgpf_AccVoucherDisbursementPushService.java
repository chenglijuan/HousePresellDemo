...
@Service
@Transactional
public class Tgpf_AccVoucherDisbursementPushService
{
	@Autowired
	...
	public Properties execute(Tgpf_AccVoucherForm model)
	{
		...
		{
			billTimeStap = mydate.dateToSimpleString(System.currentTimeMillis());
		...
		Integer accVoucherCount = tgpf_AccVoucherDao.findByPage_Size(
				tgpf_AccVoucherDao.getQuery_Size(tgpf_AccVoucherDao.getSpeicalHQL(), tgpf_AccVoucherForm));
		...
		if (accVoucherCount > 0)
		{
			...
			for (Tgpf_AccVoucher tgpf_AccVoucher : tgpf_AccVoucherList)
			{
				if("".equals(key))
				{
				...
				/*try
				{
					returnMap = tgpf_AccVoucherDao.postPZ(tgpf_AccVoucher.getTableId().toString(), "2", username);
				}
			...
			try
			{
				returnMap = tgpf_AccVoucherDao.postPZ(key, "2", username);
			}
			catch (SQLException e)
			{
				System.out.print(e.getMessage());
	 ...
	 * @return
	 */
	public ArrayList<String> getbuildingMsg(Empj_ProjectInfo project ,Empj_BuildingInfo building,Connection conn) throws SQLException
	{
		//先查询项目是否存在不存在则插入楼幢信息
		//查询楼幢不存在才插入项目信息
	 ...
	 * @throws SQLException
	 */
	public String insertPzml(String bfdate,String loanAmount,String userName,String bankName , Connection conn) throws SQLException
	{
		// 初始化凭证目录编号
		String vou_no = "";
		// 当前日期
		...
		String projectSql = "select nvl(max(substr(VOU_NO,7,4)),0)+1 maxcode from gl_vou_head where VOU_NO like 'JZ-"
				+ bfdate.substring(5, 7) + "-%' and co_code='001012001' and fiscal='" + bfdate.substring(0, 4)
				+ "'";// 查不到为0000开始+1
		PreparedStatement ps = conn.prepareStatement(projectSql);
		ResultSet rs = ps.executeQuery();
		...
		String insertProjectSql = "insert into gl_vou_head (VOU_NO, CO_CODE, ACCOUNT_ID, FISCAL, FIS_PERD, VOU_TYPE_CODE, VOU_DESC, VOU_DATE, AMT_CR, ACCE_CNT, AMT_DR, INPUTOR,  "
				...
				+ " CHECKER_DATE, ERR_MESSAGE, ERR_FLAG, ERROR) values ('JZ-"+ bfdate.substring(5, 7) +"-"+vou_no+"', '"+stableCode+"', '001', "
				+ "'"+ bfdate.substring(0,4)+"', '"+ bfdate.substring(5, 7)+"', 'JZ', "
				+ "'"+bfdate+"-"+bankName+"-拨付"+"', to_date('"+bfdate+"', 'yyyy-mm-dd'), "
	 ...
	 * @return
	 */
	public int getVou_Seq(String VOU_NO,String bfdate,Connection conn) throws SQLException
	{
		int vou_Seq = 1;
		// 查询凭证是否存在记录
		String vou_SeqSql = "select nvl(max(VOU_SEQ),0) maxcode from gl_vou_detail where  co_code='001012001' and "
				+ "VOU_NO='JZ-"+bfdate.substring(5, 7)+"-"+ VOU_NO +"' and fiscal='"+bfdate.substring(0, 4)+"' ";
		PreparedStatement ps = conn.prepareStatement(vou_SeqSql);
		ResultSet rs = ps.executeQuery();

		if (rs.next())
		{
//			if( 0 != rs.getInt("maxcode"))
//			{
			vou_Seq = rs.getInt("maxcode") + 1;
				
			String fix_SegsSql = "select FIX_SEGS from AS_NO  "
						+ "where FIX_SEGS like '001012001"+bfdate.substring(0, 4)+"001_APP_JZ-"+bfdate.substring(5, 7)+"-%' ";
			...
			{				
				...
				String insertProjectSql = "update AS_NO set CUR_INDEX = '"+ VOU_NO +"'  "
							+ " where COMPO_ID='GL_VOU_HEADVOU_NO' and FIX_SEGS='001012001"+bfdate.substring(0, 4)+"001_APP_JZ-"+bfdate.substring(5, 7)+"-'";
			...
			else
			{
				...
				String insertProjectSql = "insert into AS_NO (COMPO_ID,FIX_SEGS,CUR_INDEX)   "
							+ " values ('GL_VOU_HEADVOU_NO','001012001"+bfdate.substring(0, 4)+"001_APP_JZ-"+bfdate.substring(5, 7)+"-','1')  ";				
				PreparedStatement ps4 = conn.prepareStatement(insertProjectSql);
				ps4.executeUpdate();
			}
		}
	...
	{
		String bfdate = arr[0];
		String subjCode = arr[1];
		String loanAmount = arr[2];
				
		String vou_SeqSql = "select max(substr(VOU_NO,7,4)) maxcode from gl_vou_head where VOU_NO like 'JZ-"+bfdate.substring(5, 7)+"-%' "
				+ "and co_code='001012001' and fiscal='"+bfdate.substring(0, 4)+"' ";
		PreparedStatement ps = conn.prepareStatement(vou_SeqSql);
		ResultSet rs = ps.executeQuery();
		...
		if (rs.next())
		{
			...
			int vou_Seq = getVou_Seq(vou_No, bfdate, conn);
			...
			String insertProjectSql = "insert into gl_vou_detail (VOU_NO, CO_CODE, ACCOUNT_ID, FISCAL, VOU_SEQ, REF, DESCPT, ACC_CODE, DR_CR, STAD_AMT,  "
					...
					+ "values ('JZ-"+bfdate.substring(5, 7)+"-"+vou_No+"', '"+ stableCode +"', '001', '"+bfdate.substring(0,4)+"',  "
					+ "'"+vou_Seq+"', null, '"+bfdate+"-拨付资金"+"', '"+subjCode+"', 1,  "
					+ "'"+loanAmount+"', '"+bfdate.substring(5, 7)+"',  "
					+ "'"+bfdate+"-拨付资金"+"', 'Open', 'N', null,  "
					+ "to_date('"+bfdate+"', 'yyyy-mm-dd'), null)";
			PreparedStatement ps2 = conn.prepareStatement(insertProjectSql);
			ps2.executeUpdate();
		}
	}
	...
	public void insertPzInfo2(String[] arr,ArrayList<String> bascodes ,Connection conn) throws SQLException
	{
		...
		if (rs.next())
		{
			...
			String insertPzInfoSql = "insert into gl_vou_detail (VOU_NO, CO_CODE, ACCOUNT_ID, FISCAL, VOU_SEQ, REF, DESCPT, ACC_CODE, DR_CR, STAD_AMT,  "
					+ "FIS_PERD, VOU_DESC, STATUS, IS_PREPOSTED, OPPO_ACC_CODE, VOU_DATE, RELATION_FLAG)  "
					+ "values ('JZ-"+bfdate.substring(5, 7)+"-"+vou_No+"', '"+ stableCode +"', '001', '"+bfdate.substring(0,4)+"',  "
					+ "'"+vou_Seq+"', null, '"+bfdate+"-"+projectName+"-"+buildingName +"-拨付资金"+"', '2122', 1,  "
					+ "'"+loanAmount+"', '"+bfdate.substring(5, 7)+"',  "
					+ "'"+bfdate+"-"+projectName+"-"+buildingName +"-拨付资金"+"', 'Open', 'N', null,  "
					+ "to_date('"+bfdate+"', 'yyyy-mm-dd'), null)";
			...
			String insertPzHsInfoSql = "insert into gl_vou_detail_ass (VOU_NO, CO_CODE, ACCOUNT_ID, FISCAL, VOU_SEQ, VOU_DETAIL_SEQ, B_ACC_CODE, DR_CR, CUR_CODE, EX_RATE, CURR_AMT,  "
					...
					+ "ACC_CODE, RELATION_FLAG, DETAIL_RELATION_FLAG) values ('JZ-"+bfdate.substring(5, 7)+"-"+vou_No+"', '"+stableCode+"', '001', '"+bfdate.substring(0,4)+"',  "
					+ "'"+ vou_Seq +"', 1, '*', 1, 'RMB', 1.0000, null, '"+loanAmount+"', null, null, '*', '*', '"+companyCode+"', '*', '*', '*', '*', '*', '*',  "
					+ "null, null, '*', '*', '*', '*', '*', '*', '*', '*', '"+builditem_code+"', '*', '"+buildITEM_ISN+"', '"+bfdate.substring(5, 7)+"', 2122, null, null)";
