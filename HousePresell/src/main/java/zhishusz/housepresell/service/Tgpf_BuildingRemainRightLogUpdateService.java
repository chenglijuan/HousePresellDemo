...
/*
 * Service更新操作：楼栋每日留存权益计算日志
 * Company：ZhiShuSZ
 * */
@Service
@Transactional
public class Tgpf_BuildingRemainRightLogUpdateService
{
	@Autowired
	...
	public Properties execute(Tgpf_BuildingRemainRightLogForm model)
	{
		...
		{
			return MyBackInfo.fail(properties, "'buildingAccount(Id:" + buildingAccountId + ")'不存在");
		}
		...
		{
			return MyBackInfo.fail(properties, "'buildingExtendInfo(Id:" + buildingExtendInfoId + ")'不存在");
		}
