...
@Service
@Transactional
public class Tgxy_EscrowAgreementUpdateService
{
	@Autowired
	...
	public Properties execute(Tgxy_EscrowAgreementForm model)
	{
		 ...
		 * 
		 */
		Long tableId = model.getTableId();// 修改的主键
		...
		{
			return MyBackInfo.fail(properties, "选择协议不能为空");
		}
		...
		// 查询修改对象
		Tgxy_EscrowAgreement tgxy_EscrowAgreement = tgxy_EscrowAgreementDao.findById(tableId);
		if (null == tgxy_EscrowAgreement)
		{
			return MyBackInfo.fail(properties, "该信息无效，请刷新后重试");
		}
		...
		/*
		 * xsz by time 2018-8-30 18:43:14
		 * 设置项目相关信息：
		...
		/*
		 * xsz by time 2018-8-30 18:47:39
		 * 保存子表楼幢信息
		 * 
		 */
		...
		for (int i = 0; i < idArr.length; i++)
		{
			...
			/*
			 * xsz by time 2018-9-28 13:58:24
			 * 暂时将保存楼幢时，设置为托管状态，利于测试
			 * 后期修改掉
			 */
			/*empj_BuildingInfo.setBusiState("1");
			empj_BuildingInfoDao.save(empj_BuildingInfo);
			*/
			buildingInfoList.add(empj_BuildingInfo);

			// 楼幢号 为 施工编号
			// if (null == empj_BuildingInfo.geteCodeOfLand() ||
			// empj_BuildingInfo.geteCodeOfLand().trim().isEmpty())
			// {
			// return MyBackInfo.fail(properties, "楼幢Id:" + idArr[i] +
			// "楼编号（国土）为空");
			// }
			// str.append(empj_BuildingInfo.geteCodeOfLand() + ";");
		...
		// 设置操作人、操作时间相关信息
		tgxy_EscrowAgreement.setUserUpdate(userUpdate);
		// 设置当前时间
		...
		// tgxy_EscrowAgreement.setBusinessProcessState("1");// 业务流程状态 1--保存
		// 2--提交
		...
		if (null != smAttachmentJson && !smAttachmentJson.trim().isEmpty())
		{
			...
			for (Sm_Attachment sm_Attachment : gasList)
			{
				...
				// sm_Attachment.setBusiType("Tgxy_EscrowAgreement");
				sm_AttachmentDao.save(sm_Attachment);
			}
		}
