...
@Service
@Transactional
public class Tgpf_CyberBankStatementDtlAddService111
{
	@Autowired
...
//		}
		...
		case "26"://渤海银行
		    flag = this.BHBankListToBean(read, userStart);
		    break;
     ...
     */
    @SuppressWarnings("rawtypes")
    public Integer BHBankListToBean(List list, Sm_User userStart)
    {
        // 判断记账状态
        Integer flag = 0;
            ...
            for (int i = 11; i < list.size(); i++)
            {
                ...
                if (null != stringsList && stringsList.size() > 0)
                {
                    ...
                    else
                    {
                        ...
                        // 交易日期
                        String stringDateFormat = ((String) stringsList.get(0)).trim();
                        ...
                        // 备注摘要
                        String remark = (String) stringsList.get(8);
			...
			for (int i = 5; i < list.size(); i++)
			{
				...
				if (null != stringsList && stringsList.size() > 0)
				{
					...
					else
					{
						...
						if(null == remark || remark.trim().isEmpty() || !(remark.trim().equals("转账"))){
							continue;
						}
				...
				if (null != stringsList && stringsList.size() > 0)
				{
					...
					else
					{
						...
						tgpf_CyberBankStatementDtl
								.setTradeTimeStamp(tradeTimeStamp.trim());
						// 对方账号
						String recipientAccount = (String) stringsList.get(1);
						tgpf_CyberBankStatementDtl.setRecipientAccount(recipientAccount);
						// 对方账号名称
						String recipientName = (String) stringsList.get(2);
						tgpf_CyberBankStatementDtl.setRecipientName(recipientName);
						// 备注摘要
			            String remark = stringsList.get(7).toString().trim();
