...
@Service
@Transactional
public class Emmp_CompanyInfoUpdateService
{
	private static final String ADD_BUSI_CODE 	= "020101"; //代理公司新增
	private static final String UPDATE_BUSI_CODE= "020102"; //代理公司变更
	...
	public Properties execute(Emmp_CompanyInfoForm model)
	{
		...
		Emmp_CompanyInfo emmp_CompanyInfo = emmp_CompanyInfoDao.findByIdWithClear(emmp_CompanyInfoId);
		...
		{
			return MyBackInfo.fail(properties, "开发企业名称不能为空");
		}
		// 检查机构名称唯一性
		...
		{
			return MyBackInfo.fail(properties, "统一社会信用代码不能为空");
		}
		...
		{
			return MyBackInfo.fail(properties, "地址不能为空");
		}
		if(registeredDateStr == null || registeredDateStr.length() == 0)
		{
			return MyBackInfo.fail(properties, "请选择成立日期");
		}
...
//		}
		...
		emmp_orgMemberForm.setBankId(emmp_CompanyInfoId);
		...
		Emmp_CompanyInfoTemplate companyOldTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListOrg);
...
//		model.setOrgMemberList(orgMemberList);
		...
		if(S_BusiState.NoRecord.equals(emmp_CompanyInfo.getBusiState()))
		{
			...
			if(S_ButtonType.Submit.equals(buttonType))
			{
				...
				if(!"noApproval".equals(properties.getProperty(S_NormalFlag.info)))
				{
					...
					if(null == emmp_CompanyInfo.getTableId())
					{
						emmp_CompanyInfo.setTableId(emmp_CompanyInfoId);
					}
					//审批操作
		...
		else if(!flag)
		{
			...
			if("noApproval".equals(properties.getProperty(S_NormalFlag.info)))
			{
				...
				saveOrgmemberList(model);
				sm_AttachmentBatchAddService.execute(model, emmp_CompanyInfoId);
//				companyInfoTemplate.setTheType(S_CompanyType.Development);
//				companyInfoTemplate.setCompany(emmp_CompanyInfo);
//				companyInfoTemplate.setCityRegionName(emmp_CompanyInfo.getCityRegion().getTheName());
//				companyInfoTemplate.setStreetName(emmp_CompanyInfo.getStreet().getTheName());
//				companyInfoTemplate.setSetupTimeString(myDatetime.dateToSimpleString(emmp_CompanyInfo.getRegisteredDate()));
				...
				Emmp_CompanyInfoTemplate companyNewTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListNew);
//				companyInfoTemplate.setOrgMemberList(orgMemberListNew);
//				Emmp_CompanyInfoTemplate companyNewTemplate = ObjectCopier.copy(companyInfoTemplate);
