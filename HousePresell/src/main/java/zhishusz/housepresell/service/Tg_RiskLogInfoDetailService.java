...
@Service
@Transactional
public class Tg_RiskLogInfoDetailService
{
	// 项目风险日志
	...
	public Properties execute(Tg_RiskLogInfoForm model)
	{
		...
		if (tg_RiskLogInfo == null || S_TheState.Deleted.equals(tg_RiskLogInfo.getTheState()))
		{
			return MyBackInfo.fail(properties, "'风险日志(Id:" + tg_RiskLogInfoId + ")'不存在");
		}

		// 查询合作备忘录相关附件信息，设置附件model条件信息，进行附件条件查询 (先使用 A 方案 后期使用B方案)
		...
		properties.put("tg_RiskLogInfo", tg_RiskLogInfo);
