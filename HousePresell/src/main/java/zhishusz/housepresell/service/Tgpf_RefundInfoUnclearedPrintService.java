...
/*
 * Service打印：退房退款-贷款未结清 输出PDF打印
 * Company：ZhiShuSZ
 */
@Service
@Transactional
public class Tgpf_RefundInfoUnclearedPrintService
{
	@Autowired
	...
	String busiCode = "06120202";// 具体业务编码参看SVN文件"Document\原始需求资料\功能菜单-业务编码.xlsx"
	...
	public Properties execute(Tgpf_RefundInfoForm model)
	{
		...
		paramsHashmap.put("ajsfjq ", "否");// 按揭是否结清
		paramsHashmap.put("wdqlcqyje ", Double
				.toString(tgpf_RefundInfo.getUnexpiredAmount() == 0.00 ? 0.00 : tgpf_RefundInfo.getUnexpiredAmount()));// 未到期留存权益金额
		 ...
		 * approvalProcess_AF.getWorkFlowList();// 审批流程
		 * 
		 * for (Sm_ApprovalProcess_Workflow sm_ApprovalProcess_Workflow :
		 * workFlowList)
		 * {
		 * Sm_Permission_Role sm_Permission_Role =
		 * sm_ApprovalProcess_Workflow.getRole();//审批角色
		 * 
		 * //判断用户角色
		 * 
		 * }
		...
		// 财务部意见
		paramsHashmap.put("cwhkyh",
				tgpf_RefundInfo.getRefundBankName() == null ? "" : tgpf_RefundInfo.getRefundBankName());// 本次划款银行
		paramsHashmap.put("cwhkzh",
				tgpf_RefundInfo.getRefundBankAccount() == null ? "" : tgpf_RefundInfo.getRefundBankName());// 本次划款账户
		paramsHashmap.put("cwhksqje",
				Double.toString(tgpf_RefundInfo.getRefundAmount() == 0.00 ? 0.00 : tgpf_RefundInfo.getRefundAmount()));// 本次划款申请金额
