...
public class Empj_HouseInfoListForProjectService
{
	@Autowired
	...
	public Properties execute(Empj_HouseInfoForm model)
	{
		...
		{
			totalCount = empj_HouseInfoDao.findByPage_Size(empj_HouseInfoDao.getQuery_Size(empj_HouseInfoDao.getExcelListForProjectHQL(), model));
		}
		else
		{
			totalCount = 0;
		...
		if(totalCount > 0)
		{
			empj_HouseInfoList = empj_HouseInfoDao.findByPage(empj_HouseInfoDao.getQuery(empj_HouseInfoDao.getExcelListForProjectHQL(), model), pageNumber, countPerPage);
		}
		else
		{
