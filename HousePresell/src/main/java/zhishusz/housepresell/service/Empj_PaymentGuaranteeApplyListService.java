...
public class Empj_PaymentGuaranteeApplyListService
{
	@Autowired
	...
	public Properties execute(Empj_PaymentGuaranteeForm model)
	{
		...
		model.setApply("支付保证申请");
		
		Integer totalCount = empj_PaymentGuaranteeDao.findByPage_Size(empj_PaymentGuaranteeDao.getQuery_Size(empj_PaymentGuaranteeDao.getBasicHQL(), model));
		...
		if(totalCount > 0)
		{
			empj_PaymentGuaranteeList = empj_PaymentGuaranteeDao.findByPage(empj_PaymentGuaranteeDao.getQuery(empj_PaymentGuaranteeDao.getBasicHQL(), model), pageNumber, countPerPage);
		}
		else
		{
