 ...
 * 支付保证申请添加service
 * 
 * @author wang create by 2018/09/25
 */
@Service
@Transactional
public class Empj_PaymentGuaranteeAddService {
	@Autowired
	...
	public Properties execute(Empj_PaymentGuaranteeForm model) {
		...
		Empj_PaymentGuaranteeForm empj_PaymentGuaranteeForm = JSON.parseObject(model.getEmpj_PaymentGuarantee(),
				Empj_PaymentGuaranteeForm.class);
		...
		String guaranteeType = empj_PaymentGuaranteeForm.getGuaranteeType();// 支付保证类型(1-银行支付保证
																			// ,2-
																			// 支付保险
																			// ,3-
																			// 支付担保
		...
		empj_PaymentGuarantee.setUserStart(user);
		empj_PaymentGuarantee.setCreateTimeStamp(System.currentTimeMillis());
		...
		List<Empj_PaymentGuaranteeChildForm> empj_PaymentGuaranteeChildList = JSON
				.parseArray(model.getEmpj_PaymentGuaranteeChildList(), Empj_PaymentGuaranteeChildForm.class);
		...
		for (Empj_PaymentGuaranteeChildForm guaranteeChildForm : empj_PaymentGuaranteeChildList) {
			...
			Empj_BuildingInfo empj_BuildingInfo = (Empj_BuildingInfo) empj_BuildingInfoDao
					.findById(empj_BuildingInfoId);
			if (empj_BuildingInfo == null || S_TheState.Deleted.equals(empj_BuildingInfo.getTheState())) {
				return MyBackInfo.fail(properties, "楼幢信息不存在");
			}
			 ...
			 */
			/*
			 * Empj_BuildingInfoForm buildingInfoForm = new
			 * Empj_BuildingInfoForm();
			 * buildingInfoForm.setTableId(empj_BuildingInfoId); Properties
			 * properties1 =
			 * checkMutexService.checkPaymentGuaranteeApply(buildingInfoForm);
			 * if(!S_NormalFlag.success.equals(properties1.get(S_NormalFlag.
			 * result))) { return properties1; }
			 */
			/**
			 * BUG#4080 互斥业务
			 */
			...
			Integer childCount = empj_PaymentGuaranteeChildDao.findByPage_Size(empj_PaymentGuaranteeChildDao
					.getQuery_Size(empj_PaymentGuaranteeChildDao.getSpeicalHQL(), empj_PaymentGuaranteeChildForm));
			...
			if (null != tgpj_BuildingAccount.getAppliedNoPayoutAmount()
					&& tgpj_BuildingAccount.getAppliedNoPayoutAmount() > 0) {
			...
			Double paymentLines = 0.0;// 支付保证封顶比例
			...
			Double paymentProportion = 0.0;// 支付保证封顶额度
			if (null != tgpj_BuildingAccount.getPaymentProportion()
					&& tgpj_BuildingAccount.getPaymentProportion() > 0) {
			...
			if (null != tgpj_BuildingAccount.getTotalAmountGuaranteed()
					&& tgpj_BuildingAccount.getTotalAmountGuaranteed() > 0) {
				totalAmountGuaranteed = tgpj_BuildingAccount.getTotalAmountGuaranteed();
			}

			Double effectiveLimitedAmount = 0.0;// 有效受限额度（元）
			if (null != tgpj_BuildingAccount.getEffectiveLimitedAmount()
					&& tgpj_BuildingAccount.getEffectiveLimitedAmount() > 0) {
			...
			if (null != tgpj_BuildingAccount.getTotalAccountAmount()
					&& tgpj_BuildingAccount.getTotalAccountAmount() > 0) {
			...
			if (null != tgpj_BuildingAccount.getCurrentLimitedRatio()
					&& tgpj_BuildingAccount.getCurrentLimitedRatio() > 0) {
				currentLimitedRatio = tgpj_BuildingAccount.getCurrentLimitedRatio();
			}

			Double nodeLimitedAmount = 0.0;//// 当前节点受限额度（元）
			if (null != tgpj_BuildingAccount.getNodeLimitedAmount()
					&& tgpj_BuildingAccount.getNodeLimitedAmount() > 0) {
			...
			if (null != tgpj_BuildingAccount.getAppropriateFrozenAmount()
					&& tgpj_BuildingAccount.getAppropriateFrozenAmount() > 0) {
			...
			/*
			 * if (result == 0 && result2 == 0) // 为0 { return
			 * MyBackInfo.fail(properties, "请输入楼幢项目建设已实际支付金额或楼幢项目建设待支付承保金额！"); }
			 */
			...
			// myDouble.doubleAddDouble(totalAmountGuaranteed,
			// amountGuaranteed);
			if (amountGuaranteed > paymentProportion) {
				totalAmountGuaranteed = myDouble.doubleAddDouble(totalAmountGuaranteed, paymentProportion);
			...
			// 托管余额 = 总入账金额 - 已拨付 - 退房退款
			Double currentEscrowFund = myDouble.doubleSubtractDouble(totalAccountAmount,
					myDouble.doubleAddDouble(payoutAmount, refundAmount));
			...
			if (currentEscrowFund > effectiveLimitedAmount) {
				...
				// 可拨付金额（元）= 总入账金额-有效受限额度-已拨付金额-拨付冻结金额-退款冻结金额 = 溢出金额
				// -拨付冻结金额-退款冻结金额
				releaseTheAmount = myDouble.doubleSubtractDouble(spilloverAmount,
						myDouble.doubleAddDouble(appropriateFrozenAmount, appliedNoPayoutAmount));
			}

			// 已落实支付保证金额需要小于等于初始受限额度*支付保证封顶比例
			/*
			 * 需求 #184 提示校验去除 if( amountGuaranteed > paymentProportion) { return
			 * MyBackInfo.fail(properties, "已落实支付保证金额需要小于等于初始受限额度*支付保证封顶比例！"); }
			 */

			/**
			 * 需求 #184 如果两个承包金额相加超过支付保证封顶额度，则已落实支付保证金额字段写入支付保证封顶额度，即已落实支付保证金额字段取
			 * min(两个承保金额相加，支付保证封顶额度)。
			 */
			if (amountGuaranteed > paymentProportion) {
				empj_PaymentGuaranteeChild.setAmountGuaranteed(paymentProportion);// 已落实支付保证金额（元）
			} else {
			...
			if (null == entity) {
				entity = empj_PaymentGuaranteeDao.save(empj_PaymentGuarantee);
			}
