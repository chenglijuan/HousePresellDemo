...
@Service
@Transactional
public class Tgpf_FundOverallPlanBatchDeleteService
{
	@Autowired
	...
	public Properties execute(Tgpf_FundOverallPlanForm model)
	{
		...
		{
			Tgpf_FundOverallPlan tgpf_FundOverallPlan = (Tgpf_FundOverallPlan)tgpf_FundOverallPlanDao.findById(tableId);
			if(tgpf_FundOverallPlan == null)
			{
				return MyBackInfo.fail(properties, "'统筹单(Id:" + tableId + ")'不存在");
			}
			...
			overallPlanAccoutForm.setFundOverallPlanId(tableId);
