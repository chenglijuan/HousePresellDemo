...
@Service
@Transactional
public class Emmp_CompanyWitnessUpdateService
{
	private static final String ADD_BUSI_CODE 	= "020106"; 
	private static final String UPDATE_BUSI_CODE= "020107"; 
	...
	public Properties execute(Emmp_CompanyInfoForm model)
	{
		...
		{
			return MyBackInfo.fail(properties, "进度见证服务单位不存在");
		}
		...
		{
			return MyBackInfo.fail(properties, "请输入见证进度服务单位名称");
		}
		...
		if (checkNameCount > 0)
		{
			return MyBackInfo.fail(properties, "见证进度服务单位名称已被占用");
		}
		...
		if(emmp_CompanyInfo == null)
		{
			return MyBackInfo.fail(properties, "要修改的见证进度服务单位不存在");
		}
		...
		Emmp_CompanyInfoTemplate companyOldTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListOrg, S_CompanyType.Witness);
...
//		model.setOrgMemberList(orgMemberList);
		...
		else if(!flag)
		{
			...
			if("noApproval".equals(properties.getProperty(S_NormalFlag.info)))
			{
				...
				Emmp_CompanyInfoTemplate companyNewTemplate = companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListNew, S_CompanyType.Witness);
