...
/*
 * Service列表查询：无人机巡查机构 Company：ZhiShuSZ
 */
@Service
@Transactional(propagation = Propagation.NOT_SUPPORTED, readOnly = true)
public class Emmp_CompanyInspectionExportExcelService {
    @Autowired
    ...
    public Properties execute(Emmp_CompanyInfoForm model) {
        ...
        if (idArr == null || idArr.length < 1) {
            emmp_CompanyInfoList = emmp_CompanyInfoDao
                .findByPage(emmp_CompanyInfoDao.getQuery(emmp_CompanyInfoDao.getBasicHQL(), model), null, null);
        } else {
            emmp_CompanyInfoList = emmp_CompanyInfoDao
                .findByPage(emmp_CompanyInfoDao.getQuery(emmp_CompanyInfoDao.getExcelListHQL(), model), null, null);
        }

        ExportToExcelUtil exportToExcelUtil = new ExportToExcelUtil();
        Properties propertiesExport =
            exportToExcelUtil.execute(emmp_CompanyInfoList, Emmp_CompanyWitnessTemplate.class, "无人机巡查机构列表信息");
