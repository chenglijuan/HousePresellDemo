...
import zhishusz.housepresell.service.Sm_PoCompareResult;
...
/*
 * Service更新操作：无人机巡查机构 Company：ZhiShuSZ
 */
@Service
@Transactional
public class Emmp_CompanyInspectionUpdateService {
    private static final String ADD_BUSI_CODE = "020141";
    private static final String UPDATE_BUSI_CODE = "020142";
    ...
    public Properties execute(Emmp_CompanyInfoForm model) {
        ...
        if (theName == null || theName.length() == 0) {
            return MyBackInfo.fail(properties, "请输入无人机巡查机构名称");
        }
        ...
        if (checkNameCount > 0) {
            return MyBackInfo.fail(properties, "无人机巡查机构名称已被占用");
        }
        ...
        if (emmp_CompanyInfo == null) {
            return MyBackInfo.fail(properties, "要修改的无人机巡查机构不存在");
        }
        ...
        List<Emmp_OrgMember> orgMemberListOrg = emmp_OrgMemberDao
            .findByPage(emmp_OrgMemberDao.getQuery(emmp_OrgMemberDao.getBasicHQL(), emmp_orgMemberForm));
        Emmp_CompanyInfoTemplate companyOldTemplate =
            companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListOrg, S_CompanyType.Inspection);
        ...
        if (flag) // 如果基本字段没有变化再看机构成员列表
        {
            ...
            if (flag) // 如果没有新增再看有没有删除
            {
                ...
                Integer totalCount = emmp_OrgMemberDao
                    .findByPage_Size(emmp_OrgMemberDao.getQuery_Size(emmp_OrgMemberDao.getBasicHQL(), theModel));
        ...
        if (S_BusiState.NoRecord.equals(emmp_CompanyInfo.getBusiState())) {
            ...
            if ("noApproval".equals(properties.getProperty(S_NormalFlag.info))) {
                ...
                saveOrgmemberList(model);
                sm_AttachmentBatchAddService.execute(model, emmp_CompanyInfoId);
                ArrayList<Emmp_OrgMember> orgMemberListNew =
                    OrgMemberConverter.orgMemberFormList2OrgMemberList(model.getOrgMemberList());
                Emmp_CompanyInfoTemplate companyNewTemplate =
                    companyLogUitl.getCopyTemplate(emmp_CompanyInfo, orgMemberListNew, S_CompanyType.Inspection);
                ...
                List<Emmp_OrgMember> emmp_OrgMemberList = emmp_OrgMemberDao
                    .findByPage(emmp_OrgMemberDao.getQuery(emmp_OrgMemberDao.getBasicHQL(), theModel), null, null);
    ...
    public Emmp_OrgMemberForm[] setOrgMemberState(Emmp_OrgMemberForm[] oldFormArray,
