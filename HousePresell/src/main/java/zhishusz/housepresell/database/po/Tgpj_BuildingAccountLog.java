 ...
 * 对象名称：楼幢账户Log表
 */
@ITypeAnnotation(remark="楼幢账户Log表")
public class Tgpj_BuildingAccountLog implements Serializable,ILogable
{
	...
	private Double buildAmountPaid;
	
	@Getter @Setter @IFieldAnnotation(remark="楼幢项目建设待支付承保累计金额（元）")
	private Double buildAmountPay;
	
	@Getter @Setter @IFieldAnnotation(remark="已落实支付保证累计金额（元）")
	private Double totalAmountGuaranteed;
	
	@Getter @Setter @IFieldAnnotation(remark="关联业务编码")
	private String relatedBusiCode;
	
	@Getter @Setter @IFieldAnnotation(remark="关联业务主键")
	private Long relatedBusiTableId;
	
	@Getter @Setter @IFieldAnnotation(remark="关联楼幢账户表")
	private Tgpj_BuildingAccount tgpj_BuildingAccount;
